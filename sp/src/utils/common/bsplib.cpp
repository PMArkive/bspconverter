...
void LoadBSPFile( const char *filename )
{
	...
	if ( g_BSPConverterOptions.m_bEnabled )
	{
		...
		for ( int i = 0; i < HEADER_LUMPS; i++ )
		{
			...
			// SP doesn't support compressed lumps, unlike any other branch...
			int iUncompressedSize = *(int*)&g_pBSPHeader->lumps[i].fourCC;
			if ( iUncompressedSize > 0 )
				Error( "Compressed lump %d, not supported", i );
		}
	}
